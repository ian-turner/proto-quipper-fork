...
main = 
  do args <- getArgs
     ...
     case args of
       ...
       [filename, option] | option == "-g" -> do
         ...
         return ()
       [filename, option] | option == "-tg" -> do
         runTop p $ catchTop error_handler (topGateCount Nothing filename Nothing)
         return ()
       ...
       [filename, option, target] | option == "-p" -> do
         ...
         return ()
       [filename, option, name] | option == "-tg" -> do
         runTop p $ catchTop error_handler (topGateCount (Just name) filename Nothing)
         return ()
       [filename, option, name] | option == "-s" -> do
         runTop p $ catchTop error_handler (printTopLevel (Just name) filename)
         return ()
       [filename, option, option2, exp] | (option == "-tg") && (option2 == "-e") ->
         do
         runTop p $ catchTop error_handler (topGateCount Nothing filename (Just exp))
         return ()
         
       [filename, option, name, option2, exp] | (option == "-tg") && (option2 == "-e") ->
         do
         runTop p $ catchTop error_handler (topGateCount (Just name) filename (Just exp))
         return ()
  ...
  where printMain fn = do
        ...
        topGateCount name file Nothing =
          do dispatch (Load False file)
             ...
             case name of
               ...
               Just n ->
                 do let rs = [g | g <- gs, (getName $ gateName g) == n]
                    liftIO $ putStrLn (n++":\n" ++ show (length rs))
        topGateCount name file (Just exp) =
          do dispatch (Load False file)
             pst <- getPState
        ...
        cmdUsage = "usage: dpq <dpq-file> [-options]\n\n" ++
                   ...
                   "option: -s [name] [-e exp] -- print the gates generated by the expresion, if the expression is empty, then print the gates generated by the last definition. \n"
